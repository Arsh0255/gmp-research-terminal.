...
# 2. Cleaning text - Keeping Max Content
...
def finbert_scores_for_text(text: str, nlp):
    if not isinstance(text, str) or len(text.strip()) < 20:
        # skiping only extremely short / empty broken text
        return None

    try:
        res = nlp(text[:6000])  # avoiding overflow
        out = res[0]
...
def run_phase3():
    ...
    # Keeping almost everything (only skip if <10 chars)
...
# 2. Defining Features and Targets (Same as PHASE 4)
...
# 3. Training Models (ONE PER TARGET) - FULL DATA
...
# 4. Building Scenario Feature row
...
# 5. Running Scenarios through all Models 
...
# Sorting columns for nicer display: SPX, 10Y, sectors
ordered_cols = []
...
# 2. Identifying policy change events
...
# 2. Aligning macro data to event dates
